...
exports.generateErrorSuggestion = generateErrorSuggestion;
exports.testAIConnection = testAIConnection;
exports.formatAISuggestion = formatAISuggestion;
exports.formatGitCommand = formatGitCommand;
exports.formatMarkdown = formatMarkdown;
exports.createBox = createBox;
exports.generateErrorSuggestionWithRateLimit = generateErrorSuggestionWithRateLimit;
exports.showAISuggestionInVSCode = showAISuggestionInVSCode;
exports.makeCommitDecisionWithAI = makeCommitDecisionWithAI;
exports.generateAICompletions = generateAICompletions;
exports.generateGhostTextSuggestion = generateGhostTextSuggestion;
exports.generateCommitMessageWithAI = generateCommitMessageWithAI;
...
function debugLog(message, data) {
    if (DEBUG_MODE) {
        logger_1.default.debug('[GitCue v0.3.5] ' + message + (data ? ': ' + JSON.stringify(data) : ''));
    }
}
...
Keep it under 200 words. Use markdown formatting. Focus on actionable solutions.`;
        ...
        if (!response || !response.text) {
            logger_1.default.error('AI response is empty or invalid: ' + JSON.stringify(response));
        ...
        logger_1.default.error('AI suggestion generation failed: ' + (error instanceof Error ? error.message : String(error)));
        // Add more detailed error logging
        if (error instanceof Error) {
            logger_1.default.error(`Error details - Name: ${error.name}, Message: ${error.message}, Stack: ${error.stack || 'No stack trace'}`);
        }
        ...
        if (error instanceof Error &&
            ...
            try {
                ...
                logger_1.default.error('Retry failed: ' + (retryError instanceof Error ? retryError.message : String(retryError)));
            }
        }
...
async function testAIConnection() {
    try {
        ...
        logger_1.default.error('AI test failed: ' + (error instanceof Error ? error.message : String(error)));
        return false;
    }
}
/**
 ...
 */
function formatGitCommand(command) {
 ...
 */
function createBox(message, title) {
 ...
 */
async function generateErrorSuggestionWithRateLimit(errorContext) {
    ...
    try {
        ...
        logger_1.default.error('AI suggestion failed', error instanceof Error ? error.message : String(error));
 ...
 */
async function showAISuggestionInVSCode(errorContext) {
    try {
        ...
        const action = await vscode.window.showInformationMessage('AI Assistant has analyzed the error and generated suggestions.', 'View Suggestions', 'Copy to Clipboard', 'Dismiss');
        ...
        logger_1.default.aiSuggestion('Generated suggestion for error: ' + errorContext.substring(0, 100) + '...');
    }
    catch (error) {
        logger_1.default.error('Failed to show AI suggestion', error instanceof Error ? error.message : String(error));
 ...
 */
async function makeCommitDecisionWithAI(gitDiff, gitStatus, threshold = 'medium', requireCompleteness = true) {
    const config = config_1.configManager.getConfig();
    if (!config.geminiApiKey) {
...
Use the make_commit_decision function to provide your analysis.`;
        ...
        logger_1.default.error('AI commit decision failed: ' + (error instanceof Error ? error.message : String(error)));
        throw error;
    }
}
/**
 ...
 */
function checkThreshold(significance, threshold) {
 ...
 */
async function generateAICompletions(currentInput, workingDirectory, recentCommands) {
    const config = config_1.configManager.getConfig();
 ...
 */
async function generateGhostTextSuggestion(currentInput, workingDirectory, recentCommands) {
    const config = config_1.configManager.getConfig();
 ...
 */
async function generateCommitMessageWithAI(gitDiff, gitStatus) {
    const config = config_1.configManager.getConfig();
    if (!config.geminiApiKey) {
        throw new Error('Gemini API key not configured');
    }
...
Use the generate_commit_message function to create a proper commit message.`;
        ...
        logger_1.default.error('AI commit message generation failed: ' + (error instanceof Error ? error.message : String(error)));
        throw error;
    }
}
