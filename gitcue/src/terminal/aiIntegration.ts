...
export class AITerminalIntegration {
  private isAiChatMode = false;
  private isAnalyzing = false;
  private activityLogger: ActivityLogger;
  private chatHistory: Array<{ user: string; ai: string; timestamp: number }> = [];
   ...
   */
  async analyzeCommandError(command: string, output: string, errorOutput?: string): Promise<ErrorAnalysisResult> {
    const config = configManager.getConfig();
    ...
    try {
      ...
      const analysis = await generateErrorSuggestion(errorContext);
      ...
      logger.error('AI error analysis failed:', error instanceof Error ? error.message : String(error));
   ...
   */
  formatErrorAnalysis(result: ErrorAnalysisResult): string {
  ...
  private async generateChatResponse(message: string): Promise<AIResponse> {
    try {
...
- Use markdown formatting for clarity`;
      ...
      const content = await generateErrorSuggestion(contextualMessage);
  ...
  private buildChatContext(message: string): string {
...
`;
  ...
  private detectError(output: string, errorOutput?: string): boolean {
  ...
  private buildErrorContext(command: string, output: string, errorOutput?: string): string {
...
Please analyze this command failure and provide specific, actionable solutions.`;
  ...
  private extractQuickFixes(content: string): string[] {
    const fixes: string[] = [];
  ...
  private formatSeparator(title: string, type: 'info' | 'ai' | 'dim' = 'info'): string {
    const colors = {
      info: '\x1b[36m',
      ai: '\x1b[35m',
      dim: '\x1b[2m'
