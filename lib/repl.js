...
export async function startRepl(error = null, suggestion = null) {
  ...
  try {
    ...
    // Main REPL loop - runs indefinitely until exit
    await runReplLoop();
...
function cleanupReplHandlers() {
  ...
  // Remove global keypress handler
  if (globalKeyHandler) {
...
async function showHelp() {
  ...
  logger.info('• Infinite command loop - never stops accepting input');
  logger.space();
}
