...
export async function startRepl(error = null, suggestion = null) {
  ...
  // Properly handle raw mode transition
  originalRawMode = process.stdin.isRaw || false;
  if (process.stdin.setRawMode) {
    process.stdin.setRawMode(false);
  }
  
  // Clear any pending input and ensure clean state
  if (process.stdin.isPaused()) {
    process.stdin.resume();
  }
  
  // Remove any existing keypress listeners to prevent conflicts
  ...
  try {
    ...
    logger.info('  • Use Ctrl+R to resume watcher');
    ...
    while (true) {
      try {
        const { command } = await inquirer.prompt([{
          ...
          prefix: '',
          // Handle Ctrl+R to resume
          transformer: (input, answers, flags) => {
            if (flags.isFinal && input === '\r') {
              return 'resume';
            }
            return input;
          }
        ...
        if (inquirerError.name === 'ExitPromptError' || inquirerError.isTtyError) {
          exitReason = 'exit';
    ...
    // Clean up and restore keyboard controls
    process.stdin.removeAllListeners('keypress');
    
    // Restore raw mode for keyboard shortcuts
    if (process.stdin.setRawMode) {
...
async function handleHelp() {
  ...
  const controlCommands = {
    ...
    'resume': 'Resume watcher (or use Ctrl+R)',
