...
import { removeDuplicateChars, cleanupStdin, setupStdin, forceExit } from './utils.js';
...
export async function startRepl(error = null, suggestion = null) {
  ...
  try {
    ...
    logger.info('  • Use Ctrl+C to force exit');
    logger.space();

    exitReason = await new Promise((resolve) => {
      rl.on('line', async (input) => {
        // Remove duplicate characters from input
        const cleanInput = removeDuplicateChars(input.trim());
        ...
        const [action, ...args] = cleanInput.split(' ');
