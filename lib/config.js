...
import { loadGitignorePatterns } from './utils.js';
...
export function getConfig() {
  ...
  // Load and merge .gitignore patterns into the ignored array
  try {
    const gitignorePatterns = loadGitignorePatterns(process.cwd());
    if (gitignorePatterns.length > 0) {
      // Merge gitignore patterns with existing ignored patterns
      // Convert array patterns to regex-compatible format for chokidar
    ...
    // Silently handle errors, falling back to default ignore patterns
    console.warn('Warning: Could not load .gitignore patterns:', error.message);
  }
