  ...
  return new Promise((resolve, reject) => {
    ...
    isGitRepository().then(isRepo => {
      ...
      watcher.on('ready', () => {
        ...
        logger.info('  Ctrl+Shift+P or "1" - Pause watching', '');
        logger.info('  Ctrl+Shift+R or "2" - Resume watching', '');
        logger.info('  Ctrl+Shift+1 or "3" - Enter interactive mode', '');
...
function setupKeyboardControls() {
  ...
  process.stdin.on('keypress', async (ch, key) => {
    ...
    // Parse hotkey configuration for cross-platform compatibility
    const pauseKey = interactiveConfig.hotkeys.pause.replace(/^(ctrl\+)?(shift\+)?/, '');
    const resumeKey = interactiveConfig.hotkeys.resume.replace(/^(ctrl\+)?(shift\+)?/, '');
    const replKey = interactiveConfig.hotkeys.enterRepl.replace(/^(ctrl\+)?(shift\+)?/, '');
    
    // Handle Ctrl+Shift combinations (cross-platform)
    if (key.ctrl && key.shift && key.name === pauseKey) {
      watching = false;
      logger.warning('⏸  Watcher paused', 'Press Ctrl+Shift+R or "2" to resume');
    }
    
    if (key.ctrl && key.shift && key.name === resumeKey) {
      watching = true;
      logger.success('▶  Watcher resumed', 'Monitoring for changes...');
    }
    
    if (key.ctrl && key.shift && key.name === replKey) {
      if (!isReplActive()) {
    ...
    // Handle alternative number key shortcuts (better terminal compatibility)
    if (!key.ctrl && !key.shift && !key.meta) {
      if (ch === interactiveConfig.hotkeys.pauseAlt) {
        watching = false;
        logger.warning('⏸  Watcher paused', 'Press "2" or Ctrl+Shift+R to resume');
      }
      
      if (ch === interactiveConfig.hotkeys.resumeAlt) {
        watching = true;
        logger.success('▶  Watcher resumed', 'Monitoring for changes...');
      }
      
      if (ch === interactiveConfig.hotkeys.enterReplAlt) {
        if (!isReplActive()) {
          logger.info('🔧 Entering interactive mode...', 'MANUAL');
          await startRepl();
        }
