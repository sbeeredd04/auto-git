...
function setupKeyboardControls() {
  ...
  if (process.stdin.isTTY) {
    ...
    process.stdin.on('data', (key) => {
      ...
      // Handle 'c' key to cancel pending commit
...
async function handlePeriodicCommit(diff, commitConfig) {
  ...
  try {
    ...
    logger.commitSummary(message, shouldPush);
...
async function startCommitBuffer(commitMessage, diff, commitConfig) {
  ...
  logger.space();
  logger.info(`Press 'c' to cancel this commit within ${bufferSeconds} seconds...`, 'CONTROL');
  ...
  const countdownInterval = setInterval(() => {
    ...
    if (remainingSeconds > 0) {
      process.stdout.write(`\r⏳ Committing in ${remainingSeconds} seconds... (Press 'c' to cancel)`);
    }
  }, 1000);
