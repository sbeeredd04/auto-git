...
function setupKeyboardControls() {
  ...
  const keyHandler = async (ch, key) => {
    ...
    if (key.ctrl && key.name === 'r') {
      // If REPL is active, let it handle the resume
      if (isReplActive()) {
        return;
      }
    ...
    if (key.ctrl && key.name === 'p') {
      ...
      switch (selectedAction) {
        case 'resume':
          ...
          // Ensure keyboard controls are restored immediately
          setupKeyboardControls();
          break;
        ...
        case 'interactive':
          ...
          // Handle REPL result and ensure proper state restoration
          ...
          // Always restore keyboard controls after REPL, regardless of state
          setupKeyboardControls();
          break;
  ...
  process.stdin._autoGitKeyHandler = keyHandler;
}

function cleanupKeyboardControls() {
  if (keyboardListenerActive) {
    // Remove the specific handler we added
    if (process.stdin._autoGitKeyHandler) {
      process.stdin.removeListener('keypress', process.stdin._autoGitKeyHandler);
      delete process.stdin._autoGitKeyHandler;
    }
    
    // Remove all keypress listeners to ensure clean state
